 ...
 const Experience:React.FC<{}> = () => {
  ...
  return (
    <section className=" text-white mt-20 mx-3" id="experience">
      ...
      <div className=" flex flex-col items-center gap-8 justify-evenly  sm:gap-0 sm:items-baseline md:flex-row  mt-8  ">
        <div className={"w-[100%] sm:w-[50%]  md:flex-row justify-center flex flex-wrap gap-9"}>
          {skills.map((skill, id) => {
            return (
        ...
        <ul className=" w-[100] gap-2 sm:w-[40%] flex flex-col sm:gap-10 ">
          {History.map((historyItem, id) => {
            return (
              <li key={id} className="flex flex-col sm:flex-row items-center gap-4  bg-gradient-to-r from-black via-secondary to-transparent  w-full p-6 rounded-sm">
                <img
